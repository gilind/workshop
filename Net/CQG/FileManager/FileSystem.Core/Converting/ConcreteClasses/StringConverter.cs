...
namespace FileSystem.Core.Converting.ConcreteClasses
{
	internal class StringConverter : IStringConverter
	{
		...
		private static void AddElementToStringCollection( IElement element, ref StringCollection stringCollection, int currentLevel, string prefix, bool isLast )
		{
			if ( currentLevel == 0 )
			...
			for ( int childIndex = 0; childIndex < element.ChildrenCount; childIndex++ )
			{
				...
				AddElementToStringCollection( child, ref stringCollection, currentLevel + 1, prefix, childIndex == element.ChildrenCount - 1 );
			}
		}
	}
}